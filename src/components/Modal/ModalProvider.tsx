...
export const ModalProvider: React.FC<{ children: ReactNode }> = ({ children }) => {
    const [modals, setModals] = useState<{ type: string; content?: React.ReactNode }[]>([]);
    ...
    return (
        <ModalContext.Provider value={{ openModal, closeModal, isModalOpen }}>
            ...
            {modals.map((modal, index) => (
                <GlobalModal key={`${modal.type}-${index}`} type={modal.type} onClose={() => closeModal(modal.type)}>
                    {modal.content}
                </GlobalModal>
            ))}
...
const GlobalModal: React.FC<GlobalModalProps> = ({ onClose, children }) => {
    return (
        ...
        <div className="fixed inset-0 flex items-center justify-center z-50" onClick={onClose}>
            <div className="bg-gray-800 rounded-lg shadow-lg p-4" onClick={(e) => e.stopPropagation()}>
